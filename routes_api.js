...
module.exports = function(app, sightings) {
	...
	*/
	app.get("/api/sighting", function(req, res) {
		...
		//
		res.send(JSON.stringify(sightings.filter(function(mTime){
			return mTime.timestamp > Date.now()-600000;
			}
			)));
	});
...
/*	app.get("/api/sightings/?", function(req, res){
		console.log ("return query search")
		if (!req.session.user) {
			res.send("error");
		return;
		}
		Id = req.query.Id;
		locStr = req.query.locStr;
		if (!Id && locStr){
			//search is for Id
			return "search for Id, not locStr";
		} else if (Id && !locStr){
			//search for locStr
			return "search for locStr not Id";
		} else if (!Id && !locStr){
			//search for locStr and Id
			return "search for locStr & Id";
		} else {
			//return error incorrect search
			return"error";
		}
	});
*/
	app.get("/api/sighting/id/:pokemonId", function(req, res) {
		//check if user is logged in
		console.log("return Pokemon para search with Id " + req.params.pokemonId);
		if (!req.session.user) {
			res.send("error");
			return;
		}
	...
	*/
	app.post("/api/sighting", function(req, res) {
		...
		var latRan = Math.floor((Math.random()*100))/1000;
		var longRan = Math.floor((Math.random()*100))/1000;
		...
		if (req.body.locStr == 'boulder'){
			lat = (40.019080 + latRan).toFixed(5);
			long = (-105.235670 + longRan).toFixed(5);
		}
		if (req.body.locStr == 'broomfield'){
			lat = (39.917662 + latRan).toFixed(5);
			long = (-105.06880 + longRan).toFixed(5);
		}
